...
func (s *APIServer) telemetry() map[string]any {
	...
	if len(snapshot.Workers) > 0 {
		...
		worker = map[string]any{
			...
			"object":          map[string]any{"id": "object-1", "position": map[string]any{"x": objectX, "y": objectY}, "velocity": map[string]any{"x": denormalize(get(6), -config.MaxHorizontalSpeed, config.MaxHorizontalSpeed), "y": denormalize(get(7), -2*config.Gravity, 2*config.Gravity)}, "mass": selected.Info["object_mass"], "friction": selected.Info["object_friction"], "break_force": selected.Info["object_break_force"], "required_grip_force": selected.Info["required_grip_force"], "safety_margin": gripForce - float64(selected.Info["required_grip_force"]), "status": status},
			...
			"approach_reward": selected.Info["approach_reward"], "contact_reward": selected.Info["contact_reward"], "grip_reward": selected.Info["grip_reward"], "lift_reward": selected.Info["lift_reward"], "lift_stall_penalty": selected.Info["lift_stall_penalty"], "delivery_reward": selected.Info["delivery_reward"], "success_reward": selected.Info["success_reward"], "reward_descent_progress": selected.Info["reward_descent_progress"], "hover_penalty": selected.Info["hover_penalty"], "smoothness_penalty": selected.Info["smoothness_penalty"], "detached_force_penalty": selected.Info["detached_force_penalty"], "under_grip_penalty": selected.Info["under_grip_penalty"], "penalty_reward": selected.Info["penalty_reward"], "total_step_reward": selected.Info["total_step_reward"], "done": selected.Outcome != framework.OutcomeRunning, "outcome": selected.Outcome,
		}
	}
