...
func (s *APIServer) telemetry() map[string]any {
	...
	if len(snapshot.Workers) > 0 {
		...
		switch {
		case float64(forceRateCommand) <= config.ReleaseActionThreshold:
			forceActionMode = "release"
		case forceRateCommand > float32(config.ActionDeadZone):
