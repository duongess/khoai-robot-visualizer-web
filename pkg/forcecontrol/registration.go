...
func validateRegistration(runtime *framework.Runtime, config Config) error {
	...
	if !config.Curriculum.Stage.Valid() || config.Curriculum.ContactStartHeightOffset < 0 || config.Curriculum.ContactStableSteps <= 0 || config.Curriculum.ContactSuccessesRequired <= 0 || config.Curriculum.AlignEpisodeStepLimit < 0 || config.Curriculum.EpisodeStepLimit < 0 {
		return errors.New("force-control curriculum must select a known stage with non-negative contact offset, positive contact requirements, and non-negative episode limits")
	}
