...
func (s *APIServer) serveAPI(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == http.MethodPut && r.URL.Path == "/api/scene":
		s.updateScene(w, r)
	case r.Method == http.MethodPost && r.URL.Path == "/api/control-mode":
		s.updateControlMode(w, r)
...
type controlModeUpdate struct {
	ControlMode forcecontrol.ControlMode `json:"control_mode"`
}

// updateControlMode changes only the command-composition contract. Replacing
// tasks starts fresh episodes, which prevents a replay transition generated by
// one mode from being continued under another mode. The learner and server are
// deliberately left running; the short pause only protects Runtime.ReplaceTask.
func (s *APIServer) updateControlMode(w http.ResponseWriter, r *http.Request) {
	var request controlModeUpdate
	...
	defer s.mu.Unlock()
	next := s.config
	next.ControlMode = request.ControlMode
	// Preserve the legacy field for callers that inspect a Config directly.
	next.Residual.Enabled = request.ControlMode == forcecontrol.ModeResidual

	wasRunning := s.runtime.Snapshot().Status == framework.RuntimeRunning
	if wasRunning {
		if err := s.runtime.Pause(); err != nil {
			s.writeError(w, http.StatusConflict, "CONTROL_MODE_REJECTED", err.Error())
			return
		}
	}
	if err := forcecontrol.UpdateRegistration(s.runtime, next); err != nil {
		...
		s.writeError(w, http.StatusBadRequest, "CONTROL_MODE_REJECTED", err.Error())
		return
	}
	s.config = next
	if wasRunning {
	...
	s.writeJSON(w, http.StatusOK, map[string]any{
		"status": "ok", "control_mode": request.ControlMode,
		"episodes_reset": true,
	})
}
...
func controlModeLabel(code float32) string {
	switch int(code) {
	case 1:
		return string(forcecontrol.ModeBaseOnly)
	case 2:
