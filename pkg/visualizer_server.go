...
func (s *APIServer) sceneConfig() map[string]any {
	...
	return map[string]any{"object": map[string]any{"id": "object-1", "position_x": config.InitialObjectX, "position_y": terrainAt(config, config.InitialObjectX) + config.ObjectHeight/2, "mass": config.InitialObjectMass, "friction": config.ObjectFriction, "break_force": config.ObjectBreakForce, "initial_vertical_velocity": 0, "width": config.ObjectWidth, "height": config.ObjectHeight}, "gantry": map[string]any{"rail_y": config.WorldMaxY, "carriage_x": config.InitialCarriageX, "gripper_y": config.InitialGripperY, "min_x": config.WorldMinX, "max_x": config.WorldMaxX, "min_y": config.WorldMinY, "max_y": config.WorldMaxY, "initial_grip_force": 0, "minimum_grip_force": 0, "maximum_grip_force": config.MaxGripForce}, "terrain": map[string]any{"points": points, "ground_friction": config.ObjectFriction, "gravity": config.Gravity}, "target": map[string]any{"position_x": config.TargetX, "width": config.TargetWidth}}
}
