...
// ControlMode selects who supplies the command reaching the physical plant.
// Switching modes resets active episodes because an episode must never contain
// transitions generated by two different action semantics.
...
// action schema so WebSocket payloads remain numeric at the framework edge.
func (mode ControlMode) code() int {
	switch mode {
	case ModeBaseOnly:
		return 1
	case ModeResidual:
...
type ResidualControlConfig struct {
	...
	TransportGain float64
	HeightGain    float64
	// Nominal speeds are world-space m/s commands issued by the FSM.
	NominalDescentSpeed float64
	NominalLiftSpeed    float64
	NominalReleaseSpeed float64
	// NominalGripMultiplier is applied to the physical friction requirement;
	// it is a base-controller prior, never an actor observation.
	NominalGripMultiplier float64
	// AlphaX and AlphaY limit normalized velocity corrections. AlphaGrip is a
	// force-setpoint correction in Newtons.
	AlphaX    float64
	AlphaY    float64
	AlphaGrip float64
	// ForceSafetyMargin reserves material headroom below break force.
	ForceSafetyMargin float64
	// Reward scales are intentionally small and only used while residual mode
	// is enabled. Terminal milestones remain in RewardConfig.
	SlipSeverityPenaltyScale float64
	HoldStabilityReward      float64
	SecureGraspBonus         float64
	DampingPenaltyScale      float64
}
...
// old callers that set Residual.Enabled still receive residual composition,
// while a caller that does not opt in remains on the raw SAC action plant.
func (config Config) EffectiveControlMode() ControlMode {
...
func DefaultConfig() Config {
	return Config{
		...
		Residual: ResidualControlConfig{
			...
			TransportGain:            2.0,
			HeightGain:               2.0,
			NominalDescentSpeed:      0.75,
			NominalLiftSpeed:         0.75,
			NominalReleaseSpeed:      0.60,
			NominalGripMultiplier:    1.20,
			AlphaX:                   0.20,
			AlphaY:                   0.15,
			AlphaGrip:                4.0,
			ForceSafetyMargin:        0.50,
			SlipSeverityPenaltyScale: 1.0,
			HoldStabilityReward:      0.05,
			SecureGraspBonus:         4.0,
			DampingPenaltyScale:      0.05,
		},
		Homeostasis: HomeostasisConfig{
